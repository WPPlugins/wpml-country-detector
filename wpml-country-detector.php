...
class WCD_Plugin_Base {
    ...
    function wcd_add_admin_JS() {
        ...
        wp_register_script('settings-script-admin', plugins_url('/js/stgscript.js', __FILE__), array('jquery'), '1.0', true);
        wp_enqueue_script('settings-script-admin');
    }
    ...
    function get_flag_url() {
        if ($this->country_code) {
		    $flag = strtolower($this->country_code);
            $flag_url = ICL_PLUGIN_URL . '/res/flags/' . $flag . '.png';
        } else {
            $flag_url = false;
        }
